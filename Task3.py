...
def get_phone_type(number):
    ...
    # check if number is fixed type
    if start_index != -1 and end_index != -1 and number[1] == '0':
        return number[start_index:end_index + 1]
    # check if number is mobile type return first 4 digits
    elif number.find(' ') != -1 and (number.startswith('7') or number.startswith('8') or number.startswith('9')):
        return number[:4]
    # check if number is Telemarketers
...
def get_list_phone_number():
    ...
    for call in calls:
        if '(080)' in call[0]:
            list_numbers.append(get_phone_type(call[1]))
    return list_numbers

def part_A():
    list_numbers = get_list_phone_number()

    set_numbers = set(list_numbers)
    sorted_list = sorted(set_numbers)
...
def part_B():
    ...
    percent = (count / len(list_numbers)) * 100
